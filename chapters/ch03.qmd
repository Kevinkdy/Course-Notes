---
title: "3장 · 다중회귀분석: 추정"
subtitle: "OLS는 사실 '수직으로 내리는 것'이다"
---
...
- **표기**: Wooldridge 7판 기준 (SST / SSE / SSR — @sec-notation 참조)
...
3. **$R^2$ 는 왜 0과 1 사이인가?** $SST = SSE + SSR$ 이라는 분해가 어디선가 툭 튀어나옵니다. (표기가 교재마다 다릅니다 — @sec-notation 참조)

이 세 개는 사실 **하나의 그림**으로 동시에 풀립니다. 이 절은 그 그림을 그립니다.

## 먼저, 공간을 바꿔야 합니다 {#sec-two-spaces}
...
이걸 **변수공간(variable space)** 이라고 부릅니다. 축이 "변수"니까요.
...
![OLS의 기하학적 해석](../figures/ols-projection.svg){#fig-projection}
...
## 이제 세 가지 질문이 풀립니다 {#sec-answers}

### 우리는 도대체 무엇을 추정하는가

$x_1$ 과 $x_2$ 를 아무리 섞어도 만들 수 있는 건 **평면 위의 점들뿐**입니다. 그런데 $y$ 는 평면 밖에 있습니다. 완벽하게 맞출 방법이 없는 거죠.
...
### 잠깐 — 표기부터 정리하고 갑시다 {#sec-notation}

::: {.callout-warning}
## SSE와 SSR은 교재마다 **정반대**입니다

이 노트는 **Wooldridge 7판의 표기**를 따릅니다.

| 무엇의 제곱합인가 | Wooldridge 7e<br/>(이 노트) | Stock & Watson | 다수의 통계학·회귀분석 교재 |
|---|---|---|---|
| 총변동 $\sum(y_i-\bar y)^2$ | **SST** | TSS | SST 또는 TSS |
| 설명된 변동 $\sum(\hat y_i-\bar y)^2$ | **SSE**<br/>*Explained* | ESS | **SSR**<br/>*Regression* |
| 잔차 변동 $\sum \hat u_i^2$ | **SSR**<br/>*Residuals* | SSR | **SSE**<br/>*Error* |

Wooldridge의 **SSE**는 "**E**xplained(설명된)" 제곱합입니다. 그런데 많은 통계학
교재에서 **SSE**는 "**E**rror(오차)" 제곱합, 즉 잔차제곱합입니다. **완전히 반대**죠.
SSR도 똑같이 뒤집힙니다 — Wooldridge에서는 **R**esiduals, 통계학 교재에서는
**R**egression입니다.
...
이 노트의 표기가 아니라요. 담당 교수가 Stock & Watson을 쓰거나 통계학 계열
교재를 쓴다면 기호가 뒤집힙니다.

소프트웨어는 이 혼동을 아예 피해 갑니다. Stata의 `regress` 출력은 `Model` /
`Residual` / `Total` 이라고 **단어로** 적고, R의 `anova()` 도 행 이름으로
구분합니다. 애매한 약자를 쓰지 않는 거죠.
:::
...
```{r}
#| label: orthogonality-r
round(t(X) %*% u, 10)
```

전부 0입니다. 상수항과도 직교한다는 건 **잔차의 합이 0**이라는 뜻이죠.
...
generate prod_educ  = educ  * uhat
generate prod_exper = exper * uhat

quietly total uhat prod_educ prod_exper
matrix D = e(b)

display "상수항 : " %14.10f D[1,1]
display "educ   : " %14.10f D[1,2]
display "exper  : " %14.10f D[1,3]
```

세 값 모두 0입니다 (부동소수점 오차 수준). 상수항과의 직교가 곧
**잔차의 합이 0**이라는 뜻입니다.
...
generate dy  = (lwage - ybar)^2
generate dyh = (yhat  - ybar)^2
generate du  = uhat^2
...
## 이 그림으로 바로 설명되는 것들 {#sec-payoffs}

이 절을 이해하면 뒤의 여러 주제가 거저 풀립니다.

### 변수를 추가하면 $R^2$ 가 절대 줄지 않는 이유

설명변수를 하나 더 넣으면 평면이 **3차원 공간으로 넓어집니다**. 넓어진 공간에서 고른 최선은, 좁았을 때의 최선보다 나쁠 수가 없습니다. 원래 평면도 그 안에 포함되어 있으니까요.

그래서 잔차는 짧아지기만 하고, $R^2$ 는 커지기만 합니다. **아무 의미 없는 난수를 넣어도 마찬가지입니다.**
...
c(
  `educ만`        = summary(lm(lwage ~ educ, wage1))$r.squared,
  `educ+exper`    = summary(lm(lwage ~ educ + exper, wage1))$r.squared,
  `+난수`         = summary(lm(lwage ~ educ + exper + junk, wage1))$r.squared
)
```

난수를 넣었는데도 $R^2$ 가 올라갑니다. **조정 $R^2$ 가 필요한 이유**가 바로 이것입니다. (Wooldridge 6.3절)

### 다중공선성이 계수만 망가뜨리는 이유

$x_1$ 과 $x_2$ 가 거의 같은 방향을 가리킨다고 해봅시다. 두 화살표가 거의 겹치는 상황이죠.

- **평면 자체는 멀쩡합니다.** 여전히 평면이고, $\hat y$ 도 문제없이 잘 찾습니다. → **예측은 괜찮습니다.**
- 그런데 그 $\hat y$ 에 도달하는 **배합 비율이 불안정**해집니다. 거의 같은 방향의 두 화살표로 특정 점을 만들려면, "$x_1$ 을 크게 $+$, $x_2$ 를 크게 $-$" 같은 조합이 무수히 많이 나옵니다. 데이터가 조금만 흔들려도 비율이 확 바뀌죠.
...
이렇게 나온 계수가 **다중회귀의 $\hat\beta_1$ 과 정확히 같습니다.** 이것이 Frisch–Waugh–Lovell 정리입니다.
...
c(
  FWL  = coef(lm(r_y ~ r_x))[2],
  다중회귀 = coef(lm(lwage ~ educ + exper, wage1))["educ"]
)
```
...
regress ry rx               // 계수가 아래와 같음
regress lwage educ exper
```

**"경력을 통제한 상태에서 교육의 효과"** 라는 말의 정체가 이것입니다. 통제한다 = 그 변수 방향의 성분을 빼버린다 = **직교하는 부분만 남긴다**. 이 그림 없이는 "통제"가 그냥 주문 같은 말로 남습니다. (Wooldridge 3.2절 "부분효과" 논의)
...
| SSE는 어느 교재에서나 잔차제곱합이다 | Wooldridge에서 SSE는 **설명된** 제곱합입니다. 통계학 교재와 정반대입니다 (@sec-notation) |
